...
## Godot 桌面窗口分辨率与系统缩放修复（已批准）

### 目标与边界

本次仅修改 Godot 工程的桌面窗口和全局响应式基础设施，不调整具体业务场景的视觉设计。目标是在 Windows 等桌面平台恢复系统原生标题栏和边框，使窗口可拖动、可调整尺寸、可最大化和最小化，同时保持移动平台既有行为。

### 实施方案

1. `project.godot` 使用 1280x720 作为设计视口与初始窗口尺寸，启用可调整窗口，设置合理的最小窗口尺寸，并继续使用 `canvas_items + expand` 让 Control 与 2D 内容适配可用视口。
2. `DesktopWindowChrome.cs` 不再强制无边框和启动最大化，也不再绘制自定义桌面标题栏；桌面窗口操作交还操作系统。`ContentTopInset` 调整为零，避免原生标题栏启用后编辑器内容仍额外空出一条顶部区域。
3. `ResponsiveManager.cs` 继续以实际可见视口分发窗口尺寸和横竖屏事件，并公开当前屏幕缩放比例。屏幕或 DPI 变化时刷新状态，但不额外叠加 UI 缩放，避免操作系统 DPI 感知与 Godot 的内容拉伸被重复计算。
4. 保留 `SettingsManager` 的嵌入式子窗口设置；它与主窗口标题栏、缩放和尺寸策略互不覆盖。

### 验收标准

1. 桌面启动后为普通可调整窗口，系统标题栏的拖动、最大化、还原、最小化和关闭均可使用。
2. 调整窗口大小或切换最大化状态时，根视口尺寸事件持续触发，使用锚点和容器的界面随可用空间重排。
3. Windows 100%、125%、150% 等显示缩放下由系统与 Godot 的 HiDPI 支持共同提供清晰显示，不出现二次放大。
4. `dotnet build EraDream.csproj` 通过，测试运行器在可用的 Godot 4.6.1 .NET 可执行文件下通过。
