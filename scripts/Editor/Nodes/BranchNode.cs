...
public class BranchNodeData : BaseNodeData
{
	...
	{
		GraphNode node = new GraphNode { Title = "条件判定", Name = Id };
		...
		LineEdit varInput = new LineEdit { Text = VariableId, PlaceholderText = "变量 ID (如 gold_count)" };
		node.AddChild(varInput);
		node.SetSlot(1, true, 0, new Color(1, 1, 1), false, 0, new Color(1, 1, 1));

		// Slot 2: 对比值
		LineEdit valInput = new LineEdit { Text = ComparisonValue, PlaceholderText = "对比值 (判定 >=)" };
		node.AddChild(valInput);

		// Slot 3: 成功输出 (右侧绿色 Port 0)
		Label successLabel = new Label { Text = "满足条件 (True) ", HorizontalAlignment = HorizontalAlignment.Right };
		node.AddChild(successLabel);
		node.SetSlot(3, false, 0, new Color(1, 1, 1), true, 0, new Color(0, 1, 0));

		// Slot 4: 失败输出 (右侧红色 Port 1)
		Label failLabel = new Label { Text = "不满足条件 (False) ", HorizontalAlignment = HorizontalAlignment.Right };
