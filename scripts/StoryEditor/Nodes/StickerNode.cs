...
namespace EraDream.StoryEditor.Nodes
{
	...
	public class StickerNodeData : BaseNodeData
	{
		...
		{
			GraphNode node = new GraphNode { Title = Tr("KEY_NODE_STICKER"), Name = Id };
			...
			container.AddChild(new Label { Text = Tr("KEY_LABEL_STICKER_SELECT") });
			...
			_actionSelector.AddItem(Tr("KEY_ACTION_HIDE"), 1);
			...
			// 可视化编辑按钮
			_btnVisualEdit = new Button { Text = Tr("KEY_BTN_VISUAL_EDIT"), CustomMinimumSize = new Vector2(0, 30) };
			_btnVisualEdit.Pressed += () => OnVisualEditRequested?.Invoke(Id);
			container.AddChild(_btnVisualEdit);

			// 隐藏时禁用编辑按钮
			_actionSelector.ItemSelected += (idx) => {
				if (_btnVisualEdit != null) _btnVisualEdit.Disabled = (idx == 1);
			};
			if (_btnVisualEdit != null) _btnVisualEdit.Disabled = ActionType == "Hide";
		...
		public override void SyncFromView(GraphNode view)
		{
			...
			if (_stickerSelector != null) StickerId = _stickerSelector.GetSelectedId();
			if (_actionSelector != null) ActionType = _actionSelector.Selected == 1 ? "Hide" : "Show";
		}
	}
}
