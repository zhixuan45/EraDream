...
namespace EraDream.StoryEditor.Nodes
{
	public class BackgroundNodeData : BaseNodeData
	{
		...
		public override void SyncFromView(GraphNode view)
		{
			...
			// 同步背景文件，添加 ⚠️ 过滤保障路径合法
			if (_bgSelector != null && _bgSelector.Selected > 0)
			{
				string text = _bgSelector.GetItemText(_bgSelector.Selected);
				BackgroundFile = text.StartsWith("⚠️") ? "" : text;
			}
			else
			{
				BackgroundFile = "";
